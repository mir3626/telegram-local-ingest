...
MVP roadmap plus Sprint 9, Sprint 10, and Sprint 11 utility foundations are complete. Sprint 12 is in progress; next implementation step is a live Telegram upload to confirm the new `original-and-translated.pdf` deliverable on mobile. Regenerate/discard button exposure is a low-priority follow-up. Real Telegram DOCX uploads on 2026-04-23 confirmed `docx_text -> language=en -> agent=codex -> output registration -> completion notification -> source cleanup`, and a later download button click confirmed live `sendDocument` delivery after the non-fatal callback acknowledgement fix.

```text
Current utility flow target: local Telegram server -> upload file -> SQLite job -> import -> raw bundle -> preprocessing -> translation-needed check -> configured agent postprocess -> original+translation PDF -> output store -> Telegram download button with actual expiry -> hidden output lifecycle callbacks.
```
