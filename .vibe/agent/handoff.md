...
Sprint 12 has started. Hidden output lifecycle callbacks now exist for future utility-bot UX: `output-discard:<output_id>` deletes the runtime output file and marks the output deleted, while `output-regenerate:<output_id>` records `output.regenerate_requested` as an interface-only event without exposing a button or re-running the agent.
...
MVP roadmap plus Sprint 9, Sprint 10, and Sprint 11 utility foundations are complete. Sprint 12 is in progress; next implementation step is operator-facing output status/polish and deciding when to expose regenerate/discard controls.
