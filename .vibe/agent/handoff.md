...
- **current sprint**: `sprint-11-codex-agent-postprocess`
...
MVP roadmap plus Sprint 9 and Sprint 10 utility foundations are complete, and Sprint 11 now has worker-integrated agent post-processing. Next implementation step is to add an operator-ready Codex command recipe/live smoke harness, then polish output lifecycle actions such as hidden regenerate/discard interfaces.

```text
Current utility flow target: local Telegram server -> upload file -> SQLite job -> import -> raw bundle -> preprocessing -> translation-needed check -> configured agent postprocess -> output store -> Telegram download button.
```
