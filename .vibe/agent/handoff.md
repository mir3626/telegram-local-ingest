...
Sprint 12 has started. Hidden output lifecycle callbacks now exist for future utility-bot UX: `output-discard:<output_id>` deletes the runtime output file and marks the output deleted, while `output-regenerate:<output_id>` records `output.regenerate_requested` as an interface-only event without exposing a button or re-running the agent. The output polish slice shows the real KST expiry deadline in both message body and download button. Non-DOCX/HWP uploads still emit mobile-friendly PDFs with a Korean/CJK-capable font when available. DOCX/HWP uploads no longer force PDF conversion: the worker prefers agent-created document outputs, otherwise writes `original-and-translated.docx` with `pandoc`; DOCX sources use `--reference-doc <source.docx>`, and HWP/HWPX sources fall back to DOCX unless the agent produces native HWP/HWPX. `npm run setup:docx-rendering` now installs/checks only `pandoc`. The agent prompt includes the fixed Business Document Translation Preset and asks for the official Anthropic `docx` skill/workflow when producing DOCX. User-facing regenerate/discard Telegram controls are documented as a low-priority follow-up after the main upload, post-process, and download flow is stable.
...
MVP roadmap plus Sprint 9, Sprint 10, and Sprint 11 utility foundations are complete. Sprint 12 is in progress; next implementation step is a live DOCX upload to confirm `original-and-translated.docx` delivery, then a non-DOCX upload to confirm the retained PDF path. Regenerate/discard button exposure is a low-priority follow-up. Real Telegram DOCX uploads on 2026-04-23 confirmed `docx_text -> language=en -> agent=codex -> output registration -> completion notification -> source cleanup`, and a later download button click confirmed live `sendDocument` delivery after the non-fatal callback acknowledgement fix.

```text
Current utility flow target: local Telegram server -> upload file -> SQLite job -> import -> raw bundle -> preprocessing -> translation-needed check -> configured agent postprocess -> DOCX/HWP-family document download or non-DOCX/HWP original+translation PDF -> output store -> Telegram download button with actual expiry -> hidden output lifecycle callbacks.
```
