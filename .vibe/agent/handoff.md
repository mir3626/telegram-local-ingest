...
MVP roadmap plus Sprint 9, Sprint 10, and Sprint 11 utility foundations are complete. Sprint 12 is in progress; next implementation step is operator-facing output status/polish. Regenerate/discard button exposure is a low-priority follow-up. Real Telegram DOCX uploads on 2026-04-23 confirmed `docx_text -> language=en -> agent=codex -> output registration -> completion notification -> source cleanup`. A download button click exposed an expired callback acknowledgement crash; callback acknowledgements are now non-fatal, but the operator should click a download button once more to confirm live `sendDocument` delivery after the fix.

```text
Current utility flow target: local Telegram server -> upload file -> SQLite job -> import -> raw bundle -> preprocessing -> translation-needed check -> configured agent postprocess -> output store -> Telegram download button -> hidden output lifecycle callbacks.
```
