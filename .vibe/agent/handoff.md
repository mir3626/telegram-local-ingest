...
MVP roadmap plus Sprint 9, Sprint 10, and Sprint 11 utility foundations are complete. Sprint 12 is in progress; next implementation step is a live DOCX upload to confirm `<source-stem>_translated.docx` delivery with `[원문]` source labeling/template preservation, then a non-DOCX upload to confirm the retained `<source-stem>_translated.pdf` path. Regenerate/discard button exposure is a low-priority follow-up. Real Telegram DOCX uploads on 2026-04-23 confirmed `docx_text -> language=en -> agent=codex -> output registration -> completion notification -> source cleanup`, and a later download button click confirmed live `sendDocument` delivery after the non-fatal callback acknowledgement fix.

```text
Current utility flow target: local Telegram server -> upload file -> SQLite job -> import -> raw bundle -> preprocessing -> translation-needed check -> configured agent postprocess -> DOCX/HWP-family `<source-stem>_translated` document download or non-DOCX/HWP `<source-stem>_translated.pdf` original+translation download with `[원문]` source section -> output store -> Telegram download button with actual expiry -> hidden output lifecycle callbacks.
```
