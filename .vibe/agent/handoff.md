...
Sprint 11 is complete. `packages/agent-adapter` builds a job-scoped prompt with explicit raw/output hard boundaries, supports `{projectRoot}`, `{promptFile}`, `{outputDir}`, `{bundlePath}`, and `{jobId}` command placeholders or stdin prompt delivery, writes prompt/work files outside the output deliverable directory, and snapshots `raw/**` before/after execution so raw mutations fail the adapter. During `INGESTING`, the worker runs the agent only when `language.detected.translationNeeded=true`, converts the agent translation plus preprocessed original text into `original-and-translated.pdf`, registers that PDF through `packages/output-store`, and sends a Korean completion message with `download:<output_id>` buttons for 24-hour downloads. `scripts/run-codex-postprocess.sh` is the operator-ready Codex wrapper, and `scripts/smoke-agent-postprocess.sh` provides `npm run smoke:agent:ready` and explicit `npm run smoke:agent:live`. `AGENT_POSTPROCESS_PROVIDER` defaults to `none`; enabling `codex` or `custom` requires `AGENT_POSTPROCESS_COMMAND`.

Sprint 12 has started. Hidden output lifecycle callbacks now exist for future utility-bot UX: `output-discard:<output_id>` deletes the runtime output file and marks the output deleted, while `output-regenerate:<output_id>` records `output.regenerate_requested` as an interface-only event without exposing a button or re-running the agent. The output polish slice now emits mobile-friendly PDF downloads, uses a Korean/CJK-capable PDF font when available, and shows the real KST expiry deadline in both message body and download button. DOCX uploads now attempt template-aware rendering first: the worker writes `original-and-translated.docx` with `pandoc --reference-doc <source.docx>`, converts it to PDF with LibreOffice, and falls back to the built-in PDF renderer if those tools fail. `npm run setup:docx-rendering` installs the required local tools when missing. User-facing regenerate/discard Telegram controls are documented as a low-priority follow-up after the main upload, post-process, and download flow is stable.
...
MVP roadmap plus Sprint 9, Sprint 10, and Sprint 11 utility foundations are complete. Sprint 12 is in progress; next implementation step is a live DOCX upload to confirm the new template-aware `original-and-translated.pdf` deliverable on mobile. Regenerate/discard button exposure is a low-priority follow-up. Real Telegram DOCX uploads on 2026-04-23 confirmed `docx_text -> language=en -> agent=codex -> output registration -> completion notification -> source cleanup`, and a later download button click confirmed live `sendDocument` delivery after the non-fatal callback acknowledgement fix.

```text
Current utility flow target: local Telegram server -> upload file -> SQLite job -> import -> raw bundle -> preprocessing -> translation-needed check -> configured agent postprocess -> template-aware original+translation PDF for DOCX uploads -> output store -> Telegram download button with actual expiry -> hidden output lifecycle callbacks.
```
