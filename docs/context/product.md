...
Local agent post-processing is disabled by default through `AGENT_POSTPROCESS_PROVIDER=none`; enabling Codex or a custom command requires an explicit `AGENT_POSTPROCESS_COMMAND`. When enabled, the worker runs the agent only for jobs whose deterministic language check says translation is needed, then stores a mobile-friendly `original-and-translated.pdf` under the 24-hour runtime output cache and sends Telegram download buttons with the actual KST expiry deadline. The PDF combines the preprocessed original text and the translated/formatted result. For DOCX uploads, the worker first tries to render through `pandoc --reference-doc` plus LibreOffice so source document styles can carry into the PDF, then falls back to the built-in PDF renderer if those tools are unavailable. The Codex recipe uses `{projectRoot}/scripts/run-codex-postprocess.sh --prompt {promptFile} --output {outputDir} --bundle {bundlePath} --job {jobId}`.
