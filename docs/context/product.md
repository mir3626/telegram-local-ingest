...
Raw bundles and wiki notes can become stale or valueless over time, but deletion must remain explicit because SQLite is the operational source of job state. The preferred hard-delete path is the managed delete CLI, `npm run tlgi -- vault delete <id>`, which starts from a job/source bundle/derived artifact identity, dry-runs by default, removes related raw/wiki/derived/runtime output files only with `--apply`, and records the deletion in SQLite as job events plus a durable tombstone instead of silently dropping history.

For Obsidian-first usage, the more natural path is vault trash. Users can move unwanted active notes into `_trash/wiki/**` from Obsidian, then run `npm run tlgi -- vault trash-apply --apply` to let the CLI move linked raw/derived evidence into `_trash/raw/**` or `_trash/derived/**`, delete runtime-only outputs, create `_trash/tombstones/<id>.md`, and record the same SQLite tombstone state. `npm run tlgi -- vault trash <path_or_id> --apply` performs the same operation without a manual Obsidian move, `vault trash-list` shows pending trash plus recorded tombstones, and `vault restore <tombstone_id> --apply` moves trash files back and removes the SQLite tombstone.
...
Manual deletion from Obsidian, `raw/**`, or `wiki/**` is treated as drift, not as the canonical deletion workflow. Moving files into `_trash/**` is treated as a pending tombstone request. `npm run tlgi -- vault reconcile [--json]` scans SQLite records plus automation run result metadata against the filesystem and reports missing raw bundle parts, missing wiki pages, active output files missing on disk, orphan raw/derived bundle folders, orphan wiki source/derived pages, and `_trash/**` entries that still need `trash-apply`. Tombstoned missing paths are reported as intentionally deleted state. The command does not recreate deleted raw bundles automatically.
