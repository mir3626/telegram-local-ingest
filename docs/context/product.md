...
Local agent post-processing is disabled by default through `AGENT_POSTPROCESS_PROVIDER=none`; enabling Codex or a custom command requires an explicit `AGENT_POSTPROCESS_COMMAND`. When enabled, the worker runs the agent only for jobs whose deterministic language check says translation is needed, then stores a 24-hour runtime output and sends Telegram download buttons with the actual KST expiry deadline. DOCX/HWP-family sources are delivered as document files, preferring agent-created `original-and-translated.docx`, `translated.docx`, `.hwp`, or `.hwpx`, then falling back to a worker-rendered `original-and-translated.docx` through `pandoc`. Non-DOCX/HWP sources are delivered as a mobile-friendly `original-and-translated.pdf` that combines the preprocessed original text and the translated/formatted result. The Codex recipe uses `{projectRoot}/scripts/run-codex-postprocess.sh --prompt {promptFile} --output {outputDir} --bundle {bundlePath} --job {jobId}`.
