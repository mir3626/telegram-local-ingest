...
The dashboard shows module enabled/available state, readiness as present/missing env names only, next due time, recent runs, run result/log text, and links to raw bundle/wiki source paths when a module result records them. It supports enable/disable, manual run, and dispatch actions. It also exposes a Server-Sent Events stream at `/events` and a cursor-based `/api/logs/tail` endpoint so the page can update module/run state and live logs without manual refresh.
