...
The dashboard shows module enabled/available state, readiness as present/missing env names only, next due time, recent runs, run result/log text, structured worker diagnostics, artifact renderer diagnostics, and links to raw bundle/wiki source paths when a module result records them. It supports enable/disable, manual run, and dispatch actions. It also exposes a Server-Sent Events stream at `/events` and a cursor-based `/api/logs/tail` endpoint so the page can update module/run state and live logs without manual refresh.

Live log targets are mapped by the server, not by arbitrary user paths. Supported targets include `worker`, `bot-api`, `ops-dashboard`, `automation:<run_id>:stdout`, `automation:<run_id>:stderr`, `artifact:<run_id>:stdout`, and `artifact:<run_id>:stderr`. If `OPS_DASHBOARD_TOKEN` is set, write actions and log/SSE reads require `x-ops-dashboard-token`, `Authorization: Bearer ...`, or the local token field/query parameter used by the page; the token itself is never shown in API state.

The UI is intentionally a single-page operations surface rather than a routed app. It uses a polished control-room visual system: compact command bar, status metric ribbon, thin bordered tool surfaces, dark live-log terminal, compact tables, and a bottom detail inspector. New operational data should be added to one of these visible zones or a new adjacent zone before introducing routing, so the common monitoring loop remains visible at a glance.

Log files remain human-readable text streams for the Live log panel. Structured debugging data belongs in SQLite and dashboard APIs: worker failures emit `worker.error` job events with serialized `{ name, message, stack, context }`, and artifact renderer failures persist `error_json` on `artifact_renderer_runs`. The dashboard summarizes recent diagnostics in its top-level diagnostics zone and shows full stack/context in the detail inspector. This keeps live tailing readable while preserving enough stack trace data for packaged deployments.

The local stack helper scripts parse `.env` as key/value data instead of shell-sourcing it. This matters for values such as `FX_CURRENCIES=USD,JPY(100),EUR,CNH`. `scripts/local-stack-status.sh` reports Bot API, worker, and ops dashboard pid health, and the log tail helpers use the same parser.
