...
Deletion is a separate lifecycle concern from mutation. While present, finalized raw bundles remain immutable. If a source package is no longer valuable, the preferred paths are the managed delete command or the vault trash command family. `vault delete` removes managed files and records a tombstone. `vault trash` and `vault trash-apply` move active vault evidence into `_trash/**`, delete runtime-only outputs, create `_trash/tombstones/<id>.md`, and record the same SQLite tombstone state. Manual filesystem deletion is supported only as drift to be detected by a deterministic reconcile command; manual Obsidian moves into `_trash/**` are treated as pending tombstone requests.

The reconcile boundary is worker/CLI-owned, not LLM-owned. It compares `source_bundles.bundle_path`, `manifest_path`, `source_markdown_path`, `.finalized`, `job_outputs.file_path`, automation raw bundle outputs, derived artifact outputs, and vault raw/wiki/trash references against the filesystem. LLMwiki lint can validate wiki graph consistency, but it must not be the authority that mutates SQLite state. SQLite tombstones and vault trash metadata remain the authority for deleted/inactive source state.
